...
export async function POST(req) {
  try {
    ...
    const ack = new Response("Webhook received", { status: 200 });
  ...
  // Send email
        try {
          const hasPythonAccess = coursesToSave.some(
            (c) => c.name === "python"
          );
          const hasJavaScriptAccess = coursesToSave.some(
            (c) => c.name === "javascript"
          );
          ...
          console.log("✅ Download email sent:", email);
        } catch (mailErr) {
          console.error("Email sending failed:", mailErr);
        }
        ...
        coursesToSave = coursesToSave.map((c) => ({
          ...c,
        ...
        if (user) {
          ...
          if (!user.certificateNumber) {
            user.certificateNumber = `WD-${uuidv4()
              .slice(0, 8)
              .toUpperCase()}`;
          }
          await user.save();
        ...
        console.log("✅ Webhook processing finished:", eventId);
      } catch (err) {
